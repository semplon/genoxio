    ...
    if ( ! class_exists( 'ReduxFrameworkPlugin' ) ) {
        ...
        class ReduxFrameworkPlugin {
            ...
            public function load_first() {
                ...
                $path = str_replace( 'class.redux-plugin.php', 'framework.php', $path );
            ...
            private static function single_activate() {
                ...
                $notices[] = __( 'Redux Framework has an embedded demo.', 'genoxio' ) . ' <a href="./plugins.php?ReduxFrameworkPlugin=demo">' . __( 'Click here to activate the sample config file.', 'genoxio' ) . '</a>';
            ...
            public function add_action_links( $links ) {
                ...
                /**
                 * return array_merge(
                 *      array( 'redux_plugin_settings' => '<a href="' . admin_url( 'plugins.php?page=' . 'redux_plugin_settings' ) . '">' . __( 'Settings', 'genoxio' ) . '</a>' ),
                 *      $links
                 * );
                 */
            }
            ...
            public function plugin_metalinks( $links, $file ) {
                if ( strpos( $file, 'framework.php' ) !== false && is_plugin_active( $file ) ) {

                    $new_links = array(
                        '<a href="' . 'http://' . 'docs.reduxframework.com/" target="_blank">' . __( 'Docs', 'genoxio' ) . '</a>',
                        '<a href="' . 'https://' . 'github.com/ReduxFramework/redux-framework" target="_blank">' . __( 'Repo', 'genoxio' ) . '</a>',
                        '<a href="' . 'http://' . 'build.reduxframework.com/" target="_blank">' . __( 'Builder', 'genoxio' ) . '</a>',
                        '<a href="' . admin_url( 'tools.php?page=redux-support' ) . '">' . __( 'Get Support', 'genoxio' ) . '</a>',
                    );

                    if ( ( is_multisite() && $this->plugin_network_activated ) || ! is_network_admin() || ! is_multisite() ) {
                        if ( $this->options['demo'] ) {
                            $new_links[3] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkPlugin=demo" style="color: #bc0b0b;">' . __( 'Deactivate Demo Mode', 'genoxio' ) . '</a></span>';
                        } else {
                            $new_links[3] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkPlugin=demo" style="color: #bc0b0b;">' . __( 'Activate Demo Mode', 'genoxio' ) . '</a></span>';
                        }
                    }
